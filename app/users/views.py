...
class CustomBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        try:
            #用户名和手机都能登录，如果用户输入用户名或者手机号和密码登录，均可
...
class SmsCodeViewset(mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if sms_status["code"] != 0:
            return Response({"mobile":status["msg"]},status=status.HTTP_400_BAD_REQUEST)
        else:
...
class UserViewset(CreateModelMixin,RetrieveModelMixin,UpdateModelMixin,
    ...
    # 这里需要动态选择用哪个序列化方式
    # 1.UserRegSerializer（用户注册），只返回username和mobile，会员中心页面需要显示更多 字段，所以要创建一个UserDetailSerializer
    # 2.问题又来了，如果注册的使用userdetailSerializer，又会导致验证失败，所以需要动态的使 用serializer
