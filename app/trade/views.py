...
from ShopProject.settings import APPID, private_key_path, ali_key_path
...
from app.user_operation.permissions import IsUserOrReadOnly


class ShoppingCartViewset(viewsets.ModelViewSet):
    """ 购物车功能 list:获取购物车详情 create： 加入购物车 delete： 删除购物记录 """
    permission_classes = (IsAuthenticated, IsUserOrReadOnly)
    authentication_classes = (JSONWebTokenAuthentication, SessionAuthentication)
    serializer_class = ShopCartSerializer

    """
    默认: http://IP:port/shopcarts/shopcarts_id/ 代表更新购物车里面id=1的信息
    预期： http://IP:port/shopcarts/good_id/代表更新商品id=x的数量
    """
...
class OrderViewset(viewsets.ModelViewSet):
    """ 订单管理 list:获取个人订单 delete: 删除订单 create： 新增订单 """
    permission_classes = (IsAuthenticated, IsUserOrReadOnly)
    # authentication_classes = (JSONWebTokenAuthentication, SessionAuthentication)
...
class AlipayView(APIView):
    def get(self, request):
        ...
        alipay = AliPay(
            appid=APPID,
            app_notify_url="http://47.104.161.55:8000/alipay/return/",
            app_private_key_path=private_key_path,
            alipay_public_key_path=ali_key_path,
            debug=True,  # 默认False,
            return_url="http://47.104.161.55:8000/alipay/return/"
        )
    ...
    def post(self, request):
        ...
        if verify_re is True:
            ...
            print('*'*20,order_sn,trade_status,trade_no)
            # 查询数据库中订单记录
            existed_orders = OrderInfo.objects.filter(order_sn=order_sn)
            for existed_order in existed_orders:
                # 订单商品项, 获取该订单里面所有的商品信息,
                # 订单里面的商品信息:  <QuerySet [<OrderGoods: 202004050120512076>, <OrderGoods: 202004050120512076>]>
                order_goods = existed_order.goods.all()
                # print("订单里面的商品信息: ", order_goods)
                # 订单支付成功，商品的销售量也对应增加。
                for ordergood in order_goods:
                    # good是OrderGoods实例化的对象， 所以没有sold_num属性的。
